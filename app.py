...
if not drive_folder_id:
    ...
    if st.button("📂 获取文件列表", key="fetch_drive_list"):
        with st.spinner("正在获取文件列表..."):
            ...
            st.sidebar.write(f"**调试**：原始抓取 {len(files)} 个文件")
            if files:
            ...
            if not zip_files:
                st.sidebar.warning("没有 .zip 文件，请检查上方调试信息")
            else:
    ...
    if st.session_state.drive_file_list_fetched and st.session_state.drive_file_list:
        ...
        selected_name = st.sidebar.selectbox("选择 ZIP", file_names, key="drive_file_select")
        ...
        if selected_file and st.sidebar.button("⬇️ 下载并导入", key="drive_import_btn"):
            try:
                ...
                st.sidebar.info(f"✅ 下载完成 ({len(zip_bytes)} bytes)")
                with zipfile.ZipFile(BytesIO(zip_bytes)) as zf:
                    st.sidebar.info("ZIP 内文件：" + str(zf.namelist()))
                    if 'manifest.json' not in zf.namelist():
                        st.sidebar.error("该 ZIP 中缺少 manifest.json")
                    else:
                        manifest = json.loads(zf.read('manifest.json').decode('utf-8'))
                        st.sidebar.info(f"manifest 中有 {len(manifest['items'])} 条记录")
                        reviews = []
                        for item in manifest['items']:
                            clip_name = item['clip_name']
                            text_name = item['text_name']
